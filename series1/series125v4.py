...
def equation42(k,l2,d,d1,m,e1,b1):
    ...
    d4 = l2*d1
    l3 = -d4+(-2*d[0]+3*d[1]+3*d[2])//(2*d1)
    l4 = d4+(-2*d[0]+3*d[1]+3*d[2])//(2*d1)
    g1 = exp1(d4*d4,g,p)
    g2 = exp1(l3*l3,g,p)
    g3 = inverse(g1,p)*g2%p
    ...
    print("\n \n in e42 ({0} , {1}) t2 = {2}   h2 = {3}".format(l3,l4,t2,h2))
...
def equation87(k,l2,l3,d2,d,d1):
    g = k.g
    a2 = k.a2
    i2 = k.i2
    p = k.p
...
d = [d1*d1//2, 2048+1*64, 1024-1*64]

e = 8*(p-1)//3

# d = [d1*d1//2, 2048+0*e, 1024+0*e]


k125 = series125(-l2,l2,k.g,k.a2,k.i2,d,d1*d1,p)

d3 = [d[1] ,d[2]]
# h1 = series25(-l2,l2,k.g,k.a2,k.i2,d3,d1*d1,p)
h1 = k.series25(-l2,l2,d3,d1*d1)
print("\n \n h1  = ",h1)

d3 = [d[0], d[2]]
# h2 = series25(-l2,l2,k.g,k.a2,k.i2,d3,d1*d1,p)
...
h1 = init2(6)
h1[0] = (t1+(p-1)*b3[125])%p
h1[1] = (t2+(p-1)*b3[126])%p
h1[2] = (t3+(p-1)*b3[127])%p
h1[3] = (t4+(p-1)*b3[128])%p
h1[4] = (t5+(p-1)*b3[147])%p
h1[5] = (t6+(p-1)*b3[148])%p
print("\n \n h1 = ",h1)

l3 = l2+1
d2 = [2,4,2]
equation87(k,l2,l3,d2,d,d1)
print("\n \n")
...
k125 = series125(-l2,l2,k1.g,k1.a2,k1.i2,[(d1*d1)//2 , d[0] , d[1]],d1*d1,p)
k1.double2(k125)
k1.variables()
