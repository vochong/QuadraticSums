...
class series:
    ...
    def __init__(self,g,p): 
        ...
        "series_d125","series22","series23","series24","series25","series125")                
    ...
    def series_d125(self,c1,c2,g,a2,i2,d,d1,p):
        h1 = init2(125)
        d2 = d[0]
        d3 = d[1]
        d4 = d[2]
...
class series4:
    ...
    def __init__(self,g,l2,d,d1,p): 
        ...
        "find4","calc2","series_d22","series_d23","series_d24","series_d25",
        "series_d125","series22","series23","series24","series25","series125",
        "equation32","equation35","equation40v2","equation81v2","equation82v2",
        "equation83v2","add","double","adjust","add2","double2","adjust2",
        "fn2","fn3","fn4","f4")                
    ...
    def equation83v2(self,l2,d,d1,d5,k25):
        ...
        for i in range(5):
            for j in range(5):
                ...
                b1[e1] = multcnvl2(m[e1],k25,p)
                b2[e1] = [l4,l5]           
 
        # 2
        d3 = [d[0]+d5[0] , d[1]+d5[1]]
        for i in range(5):
            for j in range(5):
                e1 = 5*i+j
                d2 = [s[i] , s[j]]
        ...
        for i in range(25):
            t1 = 0
            if b2[i]<b3[i]:
                h1 = k.series23(b2[i][0],b3[i][0]-d1,d1,0,1)
                h2 = k.series23(b2[i][1]+d1,b3[i][1],d1,0,1)
                t1 = (h2[0]+(p-1)*h1[0])%p
            if b2[i]>b3[i]:
                h1 = k.series23(b3[i][0],b2[i][0]-d1,d1,0,1)
                h2 = k.series23(b3[i][1]+d1,b2[i][1],d1,0,1)
                t1 = (h1[0]+(p-1)*h2[0])%p
            b4[i] = t1
    ...
    def double2(self,k125):
        ...
        if len(k.d)==2:
            ...
            t2 = k.d[1]
            if t1%2==1 or t2%2==1:
                print("\n \n error in double2 function d[0] mod and d[1] mod 2")
                print("\n should both equal 0")
            k.d = [t1//2 , t2//2]
            k.d1 = k.d1//2
            k.series = h1

    def adjust2(self,n1,n2):
        k = self
        if len(k.d)==2:
            d2 = k.d1*k.d1
            if (n1%d2!=0) or (n2%d2!=0):
                print("\n \n error in adjust2 function n1 mod d2 and n2 mod d2")
                print("\n should both equal 0")
                print("\n however n1 = {0} n2 = {1} d2 = {2}".format(n1,n2,d2))

            if (n1%d2)==0 and (n2%d2)==0:
                l2 = k.l2
                d1 = k.d1
                d = k.d
                c1 = k.d[0]+n1
                c2 = k.d[1]+n2
                if (c1<0) or (c2<0):
                    print("\n \n error in adjust2 function c1<0 and/or c2<0")
                if ((c1==0) or (c1>0)) and ((c2==0) or (c2>0)):
                    d5 = [n1 , n2]
                    k25 = k.series
                    h1 = k.equation83v2(l2,d,d1,d5,k25)
                    k.series[:] = h1
                    k.d = [c1 , c2]
    ...
    def f4(self,d1,d2):
        ...
        if (d1!=2 and d1!=4) or (abs(d2)!=2 and abs(d2)!=4):
            print("\n \n error in f4 d1 and/or d2 have invalid values")
            print(" d1 and d2 are both being set equal to 2")
            d1 = 2
            d2 = 2
        ...
        if len(k.d)==2:
            ...
            if d2>0:
                mult25(m,1,p-1,0,3,4,p)
                mult25(m,1,1,1,2,5,p)
            if d2<0:
                mult25(m,1,1,0,3,4,p)
                mult25(m,1,p-1,1,2,5,p)
                
            t1 = multcnvl2(h1,m[4],p)
            t2 = multcnvl2(h1,m[5],p)
