...
void zero (int m2, STRING ptr2);
void zero2a (int m2, int r1, int r2, STRING ptr2);
...
void equal7 (unsigned int r, unsigned int c, integer t1, STRING ptr2);
void zero2 (int c1, integer h1[]);
...
void equal3(unsigned int r, unsigned int r1, STRING ptr2);
...
void init4a (int r, int c1, STRING ptr2);
void init4b (unsigned int r, int d, int c1, int c2, STRING ptr2, integer p);
...
void initcs(int d, integer a2[], integer cs[], integer sn[], integer p);
void initcs1(int d, integer a2[], integer cs1[], integer sn1[], integer p);
void initcs2(int d, integer a2[], integer cs1[], integer sn1[], integer p);
...
void init138(int m2, int l2, integer a2[], integer a10[], integer a11[],
...
zero2a (m2,e+1,e+16,ptr2);

equal7(e+1,0,1,ptr2);
equal7(e+2,d,1,ptr2);
equal7(e+3,2*d,1,ptr2);
equal7(e+4,3*d,1,ptr2);
equal7(e+5,4*d,1,ptr2);
equal7(e+6,5*d,1,ptr2);
equal7(e+7,6*d,1,ptr2);
equal7(e+8,7*d,1,ptr2);

equal7(e+9,8*d,1,ptr2);
equal7(e+10,9*d,1,ptr2);
equal7(e+11,10*d,1,ptr2);
equal7(e+12,11*d,1,ptr2);
equal7(e+13,12*d,1,ptr2);
equal7(e+14,13*d,1,ptr2);
equal7(e+15,14*d,1,ptr2);
equal7(e+16,15*d,1,ptr2);
...
{
equal7(r,c1,1,ptr2);
equal7(r,c1+2,3,ptr2);
equal7(r,c1+4,3,ptr2);
equal7(r,c1+6,1,ptr2);
}
...
for(i=0;i<t;i++)
	{
	equal7(r+i,c1*t+i,1,ptr2);
	equal7(r+i,c2*t+i,p-1,ptr2);
	}
}
...
void initcs2(int d, integer a2[], integer cs2[], integer sn2[], integer p)
{
int i;
int t1,t2;
int a3[2] = {0};
...
unsigned int a[9] = {0,1,3,4,5,7,12,14,15};
	...
	for(i=0;i<9;i++)
		...
		t2 = 9*a[i];
		init4a(t1,t2,ptr2);
		init4a(t1+1,t2+1,ptr2);
		init4a(t1+2,t2+2,ptr2);
		}
}

void equation42(STRING ptr2, integer p)
{
	init4b (27,8,1,2,ptr2,p);
	init4b (36,8,5,6,ptr2,p);
	init4b (45,8,9,10,ptr2,p);
	init4b (54,8,13,14,ptr2,p);

	init4b (63,8,4,8,ptr2,p);
	init4b (72,8,5,9,ptr2,p);
	init4b (81,8,7,11,ptr2,p);
}
...
unsigned int t1;
	...
	for(i=0;i<6;i++)
		{
		t1 = 90+i;
		equal7(t1,0*9+i,1,ptr2);
		equal7(t1,3*9+i,1,ptr2);
		equal7(t1,12*9+i,1,ptr2);
		equal7(t1,15*9+i,1,ptr2);
		}
}
...
unsigned int t1;
	...
	for(i=0;i<6;i++)
		...
		t1 = 96+i;
		equal7(t1,0*9+i,1,ptr2);
		equal7(t1,3*9+i,p-1,ptr2);
		equal7(t1,9*9+i,4,ptr2);
		equal7(t1,12*9+i,p-1,ptr2);
		equal7(t1,15*9+i,1,ptr2);
		}
}
...
unsigned int t1;
	...
	for(i=0;i<6;i++)
		{
		t1 = 102+i;
		equal7(t1,1*9+i,p-1,ptr2);
		equal7(t1,8*9+i,1,ptr2);
		equal7(t1,11*9+i,p-1,ptr2);
		equal7(t1,13*9+i,1,ptr2);
		}
}
...
for(i=0;i<3;i++)
	{
	t = 198;
	equal7(t,0,ws1[0+i],ptr2);
	equal7(t,1,ws1[3+i],ptr2);
	equal7(t,2,ws1[3+i],ptr2);
	equal7(t,3,ws1[6+i],ptr2);

	equal7(t,4,ws1[9+i],ptr2);
	equal7(t,5,ws1[12+i],ptr2);
	equal7(t,6,ws1[12+i],ptr2);
	equal7(t,7,ws1[15+i],ptr2);

	equal7(t,8,ws1[9+i],ptr2);
	equal7(t,9,ws1[12+i],ptr2);
	equal7(t,10,ws1[12+i],ptr2);
	equal7(t,11,ws1[15+i],ptr2);

	equal7(t,12,ws1[18+i],ptr2);
	equal7(t,13,ws1[21+i],ptr2);
	equal7(t,14,ws1[21+i],ptr2);
	equal7(t,15,ws1[24+i],ptr2);
...
*/
	...
	for(j=0;j<8;j++)
		...
		equal7(200,t,h11[s],ptr2);
		equal7(200,t+1,h11[s+1],ptr2);
		equal7(200,t+2,h11[s+1],ptr2);
		equal7(200,t+3,h11[s+2],ptr2);
		}
	for(j=0;j<8;j++)
		{
		t = 4*j;
